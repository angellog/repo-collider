...
// Initialize data dirs (first call creates them)
function initData() {
  try {
...
module.exports = async (req, res) => {
  ...
  if (originalUrl === '/api/auth/register' && req.method === 'POST') {
    ...
    if (users.length >= MAX_USERS) { res.status(403).json({ error: 'Registration cap reached (100 users). Join the waitlist.' }); return; }
    ...
    res.status(200).json({ token, user: { id: user.id, email: user.email, createdAt: user.createdAt } });
    return;
  }
  ...
  if (originalUrl.startsWith('/zen-proxy/')) {
    ...
    prefix = '/gh-proxy';
    replacement = '';
  } else if (cleanUrl === '/' || cleanUrl === '' || cleanUrl === '/repo-collider.html') {
    const htmlPath = path.join(__dirname, '..', 'repo-collider.html');
    try {
    ...
    return;
  } else if (cleanUrl === '/migrate' || cleanUrl === '/migrate.html') {
